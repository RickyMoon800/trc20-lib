...
def get_trx_price(coinwatch_api_key, amount: float = 1):
    def livecoinwatch():
        ...
        payload = {"currency": "USD", "code": "TRX", "meta": True}
        headers = {'content-type': 'application/json', 'x-api-key': coinwatch_api_key}
...
class MyTron:
    ...
    def get_coin_balance(self, coin_contract: str, address: str = None):
        ...
        # [ROBUSTNESS FIX] Address validation to prevent RPC failure
    ...
    def trx_to_usdt(self, amount_trx: float, fee_limit=250):
        ...
        txn = (
            contract.functions.swapExactETHForTokens.with_transfer(self.coin_to_sun(amount_trx))(
                self.coin_to_sun(min_out), [Contract.TRX, Contract.USDT], self.public_key, int(time_window.timestamp())
            )
        ...
        logger.info('Transaction sent with status {}! URL: {}{}'.format(result['receipt']['result'], self.block_explorer_tx, tx_id))
    ...
    def usdt_to_trx(self, amount_usdt: float, fee_limit=250):
        ...
        txn = (
            contract.functions.swapExactTokensForETH(
                self.coin_to_sun(amount_usdt), self.coin_to_sun(min_out), [Contract.USDT, Contract.TRX], self.public_key, int(time_window.timestamp())
            )
            .with_owner(self.public_key)
            .fee_limit(self.coin_to_sun(fee_limit))
            .build()
    ...
    def send_trx(self, recipient: str, amount: float, fee_limit=35):
        ...
        logger.info('Transaction built!')
        result = txn.broadcast().wait()
        logger.info('Transaction {} sent! URL: {}{}'.format(tx_id, self.block_explorer_tx, tx_id))
